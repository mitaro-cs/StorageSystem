...
<table>
  ...
  <tr>
    <td align="center"><b>Новое задание</b><br><sub>легко, средне или сложно</sub></td>
...
- **Домашние задания**: неделя, календарь, просроченное. У задания есть **сложность** (легко, средне,
  сложно), файлы и комментарии. Личная отметка «сделано» видна только вам.
...
### Предметы
- Преподаватель, цвет, обложка и **иконка**. На выбор 80 иконок по разделам с поиском: Σ, атом,
  колба, код, схема сети, языки, гантель… Если не выбрать, иконка **подберётся по названию** сама.
...
- **Роли**: староста, заместитель, студент в группе; **модератор и администратор сайта** — на всём
  сайте. Администратор назначает любые роли, в том числе себе. Последнего администратора снять нельзя.
...
- **Второе устройство**: та же ссылка-приглашение предложит просто войти, регистрироваться заново не
  нужно.
- **На телефоне** страница не прыгает от касаний и сразу занимает весь экран. При запуске
  показывается короткая анимация.
...
- **Уведомления**: колокольчик и push на телефон — новые задания, новости, напоминание за сутки,
  утренняя сводка. Что присылать, выбирается переключателями.
...
Приложение хоста само проверяет выпуски на GitHub. Когда выходит новая версия, в меню значка и в
«Настройки → Сервер → Состояние» появляется «Обновить». Приложение скачает обновление, установит его и
откроется заново, данные сохранятся. У участников сайт обновится при следующем открытии. Один раз, с
версий 0.1.x на 0.2.0 и новее, установщик нужно скачать вручную.
</details>
...
**Тесты**: у каждого пакета сервера свои тесты — около 300 юнит- и интеграционных, а
`EveryPackageHasTestsTest` не даст добавить пакет без них. На фронтенде — Vitest, в браузере — 20
сценариев Playwright, включая проверку, что на телефоне ни одна страница не шире экрана. CI на
каждый коммит собирает и **по-настоящему запускает** приложение на Windows и macOS.
...
- [ ] Типы заданий (контрольная, зачёт, экзамен), режим «Сессия», мастер «Новый семестр»
- [ ] Вход по отпечатку или лицу (passkeys)
- [ ] Приложение для Android (сейчас — приложение с экрана «Домой»)
