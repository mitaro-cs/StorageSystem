...
<table>
  ...
  <tr>
    ...
    <td align="center"><b>Управление</b>: группа отдельно от сайта, люди с ролями</td>
  </tr>
  <tr>
...
- **Сессия**: зачёты и экзамены по датам, обратный отсчёт до ближайшего, аудитория и прогресс
  ...
  староста может показывать её всегда или скрыть («Управление → Семестр»).
...
- **Новости**: срочные выделяются, важные закрепляются, у каждой есть комментарии, **фото и файлы**
  (большие фото уменьшаются сами).
- **Подгруппы**: английский №1 и №2, лабораторные по подгруппам — каждый выбирает свою, и задания,
  новости и уведомления другой подгруппы ему не мешают. На главной сайт сам спросит «Какая ваша?»,
  передумать можно в меню предмета («Не мой предмет»).
...
- **Поиск** понимает русские окончания: «задача» найдёт и «задачи», «расчет» — «расчёт».
  Палитра `Ctrl + K` (на Mac — `⌘K`) для поиска и быстрых действий, `?` — все сочетания клавиш.
...
### Люди и роли
- **Люди** — в «Участниках» и в «Управлении» у старосты и администратора: все одногруппники с
...
- **Модерация** — свой раздел у администраторов, модераторов и старост: жалобы участников и
  ...
  действий. Скрыть или удалить новость можно прямо с главной — меню «…» у карточки.
...
- **Свои настройки** — шестерёнка рядом с именем: «Аккаунт» (профиль, вход и безопасность с
  резервными кодами, мои данные) отдельно от «Приложения» (оформление, уведомления, без интернета).
- **Оформление под себя**: светлая, тёмная или как в системе; пять дизайнов — Классика, Стекло,
  Объём, Сияние, Бумага; основной цвет из готовых или свой — ползунком, и **насыщенность**: от
  спокойного серого до сочного цвета, в тон красятся кнопки, фон и карточки. Своя картинка на фоне
  и значок сайта в шести цветах.
- **Большие мониторы**: колонка шире и текст крупнее, «Сегодня» — в две колонки (задания и новости),
  новости — плиткой.
- **Знакомство** после регистрации: несколько карточек о том, что где и как пользоваться. Открыть
  снова — «Настройки (шестерёнка у имени) → Приложение → Как пользоваться».
...
- **Резервные копии** раз в сутки — в папку Яндекс Диска, iCloud Drive, OneDrive, Google Диска или
  Dropbox. Восстановление в пару кликов.
- **Переезд по коду**: на новом компьютере — адрес сайта и одноразовый код со старого, и сайт
  переезжает целиком, с тем же адресом, без файлов и флешек.
- **Несколько компьютеров**: сайт работает то дома на ПК, то в вузе на ноутбуке — с теми же данными и
  адресом. Закрыли groupbase на одном — другой берёт свежие данные сам (через ваш облачный диск).
...
- **Режим управления** (только у хоста) прячет кнопки администратора, чтобы пользоваться сайтом как
  участник.
...
**4. Откройте доступ.** «Управление» → «Сервер» → «Интернет — CloudPub» → войти (бесплатная
...
3. Включите уведомления в своих настройках (шестерёнка у имени) — задания будут приходить сами.
...
сохранятся. Кнопка есть и в меню значка, и в «Управление → Версия и обновления» — там же видно, какая
...
Проще всего — по коду: на старом компьютере «Управление» → «Сервер» → «Перенос на другой компьютер»
→ «Получить код переноса»; на новом установите groupbase и на экране «Первый запуск» нажмите
«Перенести по коду» — адрес сайта и код. Сайт переедет со всеми данными, адрес останется прежним.

Или файлом: «Управление» → «Сервер» → «Резервные копии» → «Скачать». На новом компьютере установите
groupbase и на экране «Первый запуск» нажмите «Восстановить из резервной копии».

Хотите работать попеременно на двух компьютерах (дома и в вузе)? «Управление» → «Сервер» →
«Несколько компьютеров»: выберите облачный диск, который есть на обоих, а на втором компьютере при
первом запуске нажмите «Подключить» — подробности в [docs/desktop.md](docs/desktop.md).
</details>
...
Войдите паролем, откройте свои настройки (шестерёнка у имени) → «Вход и безопасность» → «Вход по отпечатку или лицу» → «Добавить ключ» и подтвердите
...
не по IP-адресу. Если адрес сайта сменится, в настройках у ключа будет пометка — добавьте его заново.
</details>
